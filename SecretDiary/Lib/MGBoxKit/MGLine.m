...
- (void)setup {
  ...
  // default font styles
  self.font = [UIFont fontWithName:@"Lato-Light" size:16];
...
+ (id)multilineWithText:(NSString *)text font:(UIFont *)font width:(CGFloat)width
  ...
  // compute min height
  CGSize minSize = [text sizeWithAttributes:@{NSFontAttributeName: font}];
      ...
      if (!label.numberOfLines) {
        ...
        if ([label respondsToSelector:@selector(attributedText)]) {
          ...
          label.size = [label.text
                            boundingRectWithSize:(CGSize){limit - used, maxHeight}
                                         options:NSStringDrawingUsesLineFragmentOrigin
